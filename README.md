...
- [x] **PR1c Phase 1A** — KVQuant math-wrapper in vLLM `FlashAttentionImpl`: nuq4 quant→dequant on K/V per-layer, `kv_cache` stays fp16 (no VRAM win yet). NIAH ✓ at 4K/32K/128K/200K/256K on Qwen2.5-7B-Instruct-1M-AWQ. 256K = natural ceiling of fp16 pool on a 24 GB 3090; Phase 1B (compact bit-packed pool + fused-dequant attention kernel) is the path to true 1M attention.
- [ ] **PR1c Phase 1B / nuq2 v3** — 4×/8× compact KV pool + new nuq2 calibration (existing nuq2 v1/v2 failed needle at layer 27, nuq4 v3 passed). Calibration running with outlier_pct=0.05.
